...
import { armEmergencyStop, isEmergencyStopBlocking } from '@/lib/cloud/personal-imaging/estop-sync'
import { emitAgentWakePollSequence } from '@/lib/imaging/live-bus'
import { fetchAscCloud } from '@/lib/imaging/asc-cloud'
...
function armWeatherSafetyEmergencyStop(
  threat: WeatherSafetyThreat,
  tenantId?: string
): WeatherSafetyArmResult {
 ...
 */
export async function maybeArmWeatherSafetyEmergencyStop(
  tenantId?: string
  ...
  const run = (async (): Promise<WeatherSafetyArmResult> => {
    try {
      ...
      return armWeatherSafetyEmergencyStop(threat, tenantId)
    } catch {
...
export function triggerWeatherSafetyEmergencyStopCheck(tenantId?: string): void {
  void maybeArmWeatherSafetyEmergencyStop(tenantId)
}
