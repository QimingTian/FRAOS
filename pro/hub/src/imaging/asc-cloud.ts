/** ASC camera status client — URL from env / site config (not hard-coded). */
...
/** Prefer HUB_ASC_STATUS_URL, else derive from HUB_ASC_STREAM_URL. */
export function configuredAscStatusUrl(): string | null {
  const direct = process.env.HUB_ASC_STATUS_URL?.trim()
  if (direct) return direct
  return allSkyCameraStatusUrl(process.env.HUB_ASC_STREAM_URL?.trim() || null)
}
