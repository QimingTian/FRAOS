  ...
  return (
    <div ref={anchorRef}>
      ...
      <nav
        ...
        className={
          stuck
            ? 'lg-header fixed inset-x-0 top-16 z-50'
            : 'lg-header relative z-50 bg-transparent'
        }
        aria-label={`${productName} sections`}
      >
        <div className="lg-glass-content page-shell flex items-center gap-4">
              ...
              {items.map((item) => {
                ...
                return (
                  <li key={item.id}>
                    <a
                      href={`#${item.id}`}
                      className={`lg-nav-pill whitespace-nowrap px-3 py-1.5 text-sm transition ${
                        active ? 'lg-nav-pill-active text-fg' : 'text-muted hover:text-fg'
                      }`}
