...
async function effectiveLicenseOrder(order: PersonalOrderRecord | null): Promise<PersonalOrderRecord | null> {
  if (!order) return null
  if (order.plan === 'max' && order.maxSubscriptionOrderId?.trim()) {
    const primary = await loadOrder(order.maxSubscriptionOrderId.trim())
    if (primary) {
      return {
        ...order,
...
export async function personalIsTenantLicenseActive(tenantId: string): Promise<boolean> {
  ...
  const effective = await effectiveLicenseOrder(order ?? null)
  return licenseIsActive(effective, hasSecret)
}
...
export async function personalLicenseSummaryForTenant(
  ...
  return {
    active: licenseIsActive(effective, hasSecret),
    ownerName,
    plan,
    ...
    nextBillAt: effective?.nextBillAt ?? null,
  }
}
