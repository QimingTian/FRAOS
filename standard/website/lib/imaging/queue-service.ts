...
function buildSequenceForSession(
  session: SessionRow,
  tenantId?: string,
...
export async function createQueueSession(
  ...
  const authPassword = tenantId ? await personalTenantSecret(tenantId) : undefined
  ...
  if (draft.projectMode) {
    ...
    const ninaSequenceJson = buildSequenceForSession(draft, tenantId, authPassword)
    if (ninaSequenceJson) {
...
export async function updatePendingSession(
  ...
  if (patched.projectMode) {
    ...
    const ninaSequenceJson = buildSequenceForSession(patched, tenantId, authPassword)
    if (ninaSequenceJson) {
      setSessionNinaSequenceJson(id, ninaSequenceJson)
    }
  }
