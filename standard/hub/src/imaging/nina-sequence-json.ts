import { firstAltitudeAllowedTimeMs, currentAltitudeDeg, MIN_ALTITUDE_DEG } from '../astro/target-altitude.js'
import { getTonightAstronomicalNightWindow } from '../astro/sunrise-window.js'
import classicSingleTemplate from '../../assets/nina/Classic DSO Imaging Sequence.json' with { type: 'json' }
import classicMultiTemplate from '../../assets/nina/Classic DSO Imaging Sequence Multi Filter.json' with { type: 'json' }
import variableStarTemplate from '../../assets/nina/Variable Star Sequence.json' with { type: 'json' }
...
export function buildNinaSequenceJson(params: NinaSequenceParams): string {
  ...
  const boreanQueueId = params.boreanQueueId?.trim() ?? ''
  if (boreanQueueId) {
    root['BoreanAstro'] = {
      QueueId: boreanQueueId,
    ...
    applySessionProgressQueueIdInHttpPosts(root, boreanQueueId, params.sessionProgressUrl)
  }
...
function applySessionProgressQueueIdInHttpPosts(
  node: unknown,
  queueId: string,
  sessionProgressUrl?: string
): void {
  const walk = (value: unknown): void => {
    ...
    if (typeName.includes('GroundStation.HTTP.HttpClient')) {
      ...
      if (typeof uri === 'string' && uri.includes('/imaging/session-progress')) {
        if (sessionProgressUrl) rec['HttpUri'] = sessionProgressUrl
        ...
        rec['HttpPostBody'] = JSON.stringify({
          queueId,
          text,
          BoreanAstro: { QueueId: queueId },
        })
      }
    }
    for (const child of Object.values(rec)) walk(child)
...
function applyNinaHttpAuthCredentials(node: unknown): void {
  ...
  if (!pass) return
  const user = process.env.NINA_SESSION_PROGRESS_BASIC_USER ?? 'boreanastro'
...
/* ------------------------------------------------------------------ */
/*  Session-shaped wrapper (ported module contract)                   */
/* ------------------------------------------------------------------ */

/**
 * Session-shaped input for {@link buildNinaSequence}. Mirrors the fields the
 * scheduling engine stores on a session row, using the ported module's
 * terminology (filters, projectMode, etc.).
 */
export type NinaSessionInput = {
  id: string
  target: string
  raHours?: number | null
  decDeg?: number | null
...
/**
 * Hub config passed into {@link buildNinaSequence}. Currently only the site
 * name and session-progress URL influence the emitted JSON; lat/lon are
 * accepted for forward compatibility but are not used to build the sequence.
 */
export type NinaBuildConfig = {
  siteName?: string
  sessionProgressUrl?: string
}

/**
 * Build a NINA sequence JSON document from a session-shaped object plus hub
 * config. This is the ported-module wrapper around {@link buildNinaSequenceJson}:
 * it normalises the session's filter plans (falling back to the single
 * filter/exposure/count fields) and delegates to the template-based builder.
 *
 * @param session session data (RA/Dec, filter plans, exposure, template kind)
 * @param config hub config (site name, session-progress URL)
 * @returns pretty-printed NINA sequence JSON string
 * @throws when the session lacks RA/Dec or filter/exposure/count, or when the
 *   shipped template structure drifts
 */
export function buildNinaSequence(session: NinaSessionInput, config?: NinaBuildConfig): string {
  const raHours = session.raHours ?? null
  const decDeg = session.decDeg ?? null
  const filterName = session.filterPlans?.[0]?.filterName ?? session.filter ?? ''
  const exposureSeconds = session.filterPlans?.[0]?.exposureSeconds ?? session.exposureSeconds ?? null
  const count = session.filterPlans?.[0]?.count ?? session.count ?? null
  if (
    raHours == null ||
    decDeg == null ||
    !filterName ||
    ...
    throw new Error(
      'buildNinaSequence: session requires raHours, decDeg, and a filter/exposureSeconds/count (or filterPlans).'
    )
  }
  const filterPlans =
    Array.isArray(session.filterPlans) && session.filterPlans.length > 0
      ? session.filterPlans.map((p) => ({
