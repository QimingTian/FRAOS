import endNightTemplate from '../../assets/nina/End Night Session.json' with { type: 'json' }
import {
  appendAuditLog,
  consumeSession,
  getEstopFlag,
...
import { isAltitudeAllowed } from '../astro/target-altitude.js'
...
import { sequenceJsonForSession } from './queue-service.js'
import {
  getProjectNightById,
...
> {
  ...
  // Generic ESTOP flag: while armed, refuse to hand out any imaging sequence
  // (HTTP 410 Gone). This coexists with the richer estop_state flow below.
  if (getEstopFlag().active) {
    return {
      kind: 'error',
      status: 410,
      error: 'Emergency STOP flag is active; no imaging sequences are available.',
    }
  }
...
export { getEmergencyStopPublicState }