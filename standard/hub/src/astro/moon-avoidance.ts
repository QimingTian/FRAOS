import { readObservatoryCoords } from '../observatory-local-time.js'
...
/* ------------------------------------------------------------------ */
/*  Convenience exports for the scheduling engine port                */
/* ------------------------------------------------------------------ */

/**
 * Angular separation (degrees) between a target and the Moon at a given UTC
 * instant. Pure astronomy math — moon position comes from the simplified
 * Meeus model in {@link moonEquatorial}, independent of observer location.
 *
 * @param targetRa target right ascension in decimal hours
 * @param targetDec target declination in decimal degrees
 * @param timeUtc UTC instant to evaluate
 * @returns great-circle separation in degrees [0, 180]
 */
export function getMoonSeparation(
  targetRa: number,
  targetDec: number,
  timeUtc: Date
): number {
  return targetMoonSeparationDeg(targetRa, targetDec, timeUtc)
}

/**
 * Check moon avoidance for a set of filters at a single instant, using the
 * Lorentzian model. Moon altitude is evaluated at the configured observatory
 * (so the below-horizon / low-altitude relaxation applies); separation is
 * location-independent. Pure — no DB writes.
 *
 * @param targetRa target right ascension in decimal hours
 * @param targetDec target declination in decimal degrees
 * @param filters filter names to check (e.g. ["L", "Ha", "OIII"])
 * @param timeUtc UTC instant to evaluate
 * @returns per-filter pass/fail plus the overall verdict
 */
export function checkMoonAvoidance(
  targetRa: number,
  targetDec: number,
  filters: readonly string[],
  timeUtc: Date
): { ok: boolean; blockedFilters: string[]; separationDeg: number } {
  const blocked: string[] = []
  for (const filterName of filters) {
    if (!moonFilterOkAt(filterName, targetRa, targetDec, timeUtc)) {
      blocked.push(normalizeFilterName(filterName) || filterName)
    }
  }
  return {
    ok: blocked.length === 0,
    blockedFilters: blocked,
    separationDeg: targetMoonSeparationDeg(targetRa, targetDec, timeUtc),
  }
}
