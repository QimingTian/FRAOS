...
/* ------------------------------------------------------------------ */
/*  Explicit-coord convenience exports (pure astronomy math)          */
/* ------------------------------------------------------------------ */

/**
 * Target altitude (degrees above horizon) at a specific UTC instant for an
 * arbitrary observer location. Pure astronomy math — no DB, no side effects.
 *
 * @param ra target right ascension in decimal hours [0,24)
 * @param dec target declination in decimal degrees [-90,90]
 * @param timeUtc UTC instant to evaluate
 * @param lat observer latitude in decimal degrees
 * @param lon observer longitude in decimal degrees (west negative)
 * @returns altitude in degrees, range [-90, 90]
 */
export function getTargetAltitude(
  ...
  return currentAltitudeDeg(ra, dec, timeUtc, { lat, lon })
}

/**
 * Whether a target stays above {@link MIN_ALTITUDE_DEG} for the full session
 * window (100% coverage of 5-minute buckets), for an arbitrary observer.
 * Pure astronomy math — no DB, no side effects.
 *
 * @param ra target right ascension in decimal hours
 * @param dec target declination in decimal degrees
 * @param startUtc session start (UTC)
 * @param durationMinutes planned session length in minutes
 * @param lat observer latitude in decimal degrees
 * @param lon observer longitude in decimal degrees (west negative)
 * @returns true when every bucket midpoint is at/above the minimum altitude
 */
export function isTargetVisibleForSession(
  ra: number,
  dec: number,
  startUtc: Date,
  ...
  if (!Number.isFinite(startMs) || durationMs <= 0) return false
  return altitudeSessionCoverageOk(ra, dec, startMs, startMs + durationMs, { lat, lon })
}
