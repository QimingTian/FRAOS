...
import { loadActiveHubTenant } from '../active-max-site'
...
export async function submitImagingSession(
  ...
  tenantOverride?: PersonalTenantConfig
): Promise<SubmitImagingSessionResult> {
  return writeImagingSession('POST', '/imaging/queue', payload, tenantOverride)
}
...
async function writeImagingSession(
  ...
  const tenant = tenantOverride ?? (await loadActiveHubTenant())
