...
function locationStorageKey(siteTenantId?: string): string {
  return `${LOCATION_KEY}.${resolveSiteTenantId(siteTenantId)}`
}

function configuredStorageKey(siteTenantId?: string): string {
  return `${CONFIGURED_KEY}.${resolveSiteTenantId(siteTenantId)}`
}
...
function readStoredLocation(siteTenantId?: string): ObservatoryLocation | null {
  const scopedKey = locationStorageKey(siteTenantId)
  try {
  ...
  try {
    const legacyRaw = localStorage.getItem(LOCATION_KEY)
    if (!legacyRaw) return null
    return normalizeLocation(JSON.parse(legacyRaw) as Partial<ObservatoryLocation>)
  } catch {
    return null
  }
}

export function getObservatoryLocation(siteTenantId?: string): ObservatoryLocation {
  const stored = readStoredLocation(siteTenantId)
...
export function setObservatoryLocation(loc: ObservatoryLocation, siteTenantId?: string): void {
  const normalized = normalizeLocation(loc)
  if (!normalized) return
  localStorage.setItem(locationStorageKey(siteTenantId), JSON.stringify(normalized))
  window.dispatchEvent(new CustomEvent(OBSERVATORY_LOCATION_CHANGED, { detail: normalized }))
}

export function isObservatoryConfigured(siteTenantId?: string): boolean {
  if (import.meta.env.DEV) return true
  const scoped = localStorage.getItem(configuredStorageKey(siteTenantId))
  if (scoped === '1') return true
  const legacyTenantId = getPersonalTenant().tenantId
  if (resolveSiteTenantId(siteTenantId) !== legacyTenantId) return false
  return localStorage.getItem(CONFIGURED_KEY) === '1'
}

export function markObservatoryConfigured(siteTenantId?: string): void {
  localStorage.setItem(configuredStorageKey(siteTenantId), '1')
}
