...
import { ACTIVE_MAX_SITE_CHANGED, getActiveMaxSiteTenantId } from '../lib/active-max-site'
import {
...
import { MaxSiteSelectField } from '../components/settings/MaxSiteSelectField'
...
export function SettingsPage() {
  const [locationSiteId, setLocationSiteId] = useState(() => getActiveMaxSiteTenantId())
  const [equipmentSiteId, setEquipmentSiteId] = useState(() => getActiveMaxSiteTenantId())
  ...
  const [sensorWidthPx, setSensorWidthPx] = useState('')
  const [sensorHeightPx, setSensorHeightPx] = useState('')
  const [positionAngleDeg, setPositionAngleDeg] = useState('0')
  const [equipment, setEquipmentState] = useState(() => getEquipment(getActiveMaxSiteTenantId()))
  ...
  const loadLocationForm = useCallback((siteTenantId: string) => {
    const observatory = getObservatoryLocation(siteTenantId)
  ...
  const loadEquipmentForm = useCallback((siteTenantId: string) => {
    const nextEquipment = getEquipment(siteTenantId)
    ...
    setEqLabel(nextEquipment.label)
    setFocalLengthMm(String(nextEquipment.focalLengthMm))
    setPixelSizeUm(String(nextEquipment.pixelSizeUm))
    setSensorWidthPx(String(nextEquipment.sensorWidthPx))
    setSensorHeightPx(String(nextEquipment.sensorHeightPx))
    setPositionAngleDeg(String(nextEquipment.positionAngleDeg))
  }, [])

  useEffect(() => {
    loadLocationForm(locationSiteId)
  }, [locationSiteId, loadLocationForm])

  useEffect(() => {
    loadEquipmentForm(equipmentSiteId)
  }, [equipmentSiteId, loadEquipmentForm])

  useEffect(() => {
    const onActiveSiteChange = () => {
      const activeId = getActiveMaxSiteTenantId()
      setLocationSiteId(activeId)
      setEquipmentSiteId(activeId)
    }
    window.addEventListener(ACTIVE_MAX_SITE_CHANGED, onActiveSiteChange)
    return () => window.removeEventListener(ACTIVE_MAX_SITE_CHANGED, onActiveSiteChange)
  }, [])
  ...
  function handleSaveEquipment(e: React.FormEvent) {
    ...
    setEquipment(mergeEquipmentManualSave(equipment, validated.equipment), equipmentSiteId)
    setEquipmentState(getEquipment(equipmentSiteId))
  ...
  function handleClearEquipment() {
    clearEquipment(equipmentSiteId)
    loadEquipmentForm(equipmentSiteId)
  ...
  function handleSaveLocation(e: React.FormEvent) {
    ...
    setObservatoryLocation(validated.location, locationSiteId)
      ...
      <div className="settings-grid">
        <section className="remote-glass-pane settings-pane settings-pane-location">
          ...
          <form className="mt-2 space-y-3" onSubmit={handleSaveLocation}>
            <MaxSiteSelectField
              label="Observatory site"
              value={locationSiteId}
              onChange={setLocationSiteId}
            />
        ...
        <section className="remote-glass-pane settings-pane settings-pane-equipment">
          ...
          <form className="mt-2 space-y-3" onSubmit={handleSaveEquipment}>
            <MaxSiteSelectField
              label="Observatory site"
              value={equipmentSiteId}
              onChange={setEquipmentSiteId}
            />
            <label className="block text-sm text-white/70">
              Rig name
              <input
