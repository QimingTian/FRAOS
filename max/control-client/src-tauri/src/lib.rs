...
struct TenantFile {
    ...
    #[serde(default)]
    max_sites: Option<Vec<MaxSiteFile>>,
}
...
fn activate_account(api_base_url: String, login: String, password: String) -> Result<TenantFile, String> {
    ...
    if let Some(sites) = license_json.get("maxSites").and_then(|value| value.as_array()) {
        let parsed: Vec<MaxSiteFile> = sites
            .iter()
            .filter_map(|site| serde_json::from_value(site.clone()).ok())
            .collect();
        if !parsed.is_empty() {
            tenant.max_sites = Some(parsed);
        }
    }
